...
class SchemaSymbol
  ...
  def adjacent_to?(part_number)
    @x.between?(part_number.start_x - 1, part_number.end_x + 1) && @y.between?(part_number.y - 1, part_number.y + 1)
  end
...
class Gear
  def initialize(numbers)
    raise "Invalid numbers" unless numbers.size == 2
  ...
  def run
    ...
    valid_parts = part_numbers.select do |part_number|
      symbols.any? do |symbol|
        symbol.adjacent_to?(part_number)
      end
    end

    stars = symbols.select { |symbol| symbol.symbol == "*" }
    gears = []

    stars.each do |star|
      adjacent_numbers = []
      valid_parts.each do |part|
        adjacent_numbers << part.number if star.adjacent_to?(part)
      end

      gears << Gear.new(adjacent_numbers) if adjacent_numbers.size == 2
    end

    puts gears.map(&:ratio).sum
  end
